...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['foto_perfil']) && $_FILES['foto_perfil']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($file['type'], $allowedTypes)) {
            ...
            $errors[] = 'A imagem é muito grande. O tamanho máximo é 2MB.';
        } else {
            // Cria um nome de ficheiro único
            $fileExtension = pathinfo($file['name'], PATHINFO_EXTENSION);
            $newFilename = uniqid('user_'.$user_id.'_') . '.' . $fileExtension;
            $newPhotoPath = $uploadDir . $newFilename;

            if (!move_uploaded_file($file['tmp_name'], $newPhotoPath)) {
                $errors[] = 'Erro ao guardar a nova imagem.';
                $newPhotoPath = null;
            }
        }
    }

    // Validação do nome e e-mail
    if (empty($nome)) {
    ...
    if (empty($errors)) {
        try {
            // Pega o caminho da foto antiga para apagar depois
            ...
            if ($newPhotoPath) {
                $sql = "UPDATE users SET username = ?, email = ?, foto_perfil = ? WHERE id = ?";
                $params = [$nome, $email, $newPhotoPath, $user_id];
            } else {
            ...
            // Se o upload foi bem-sucedido e a foto antiga existe, apaga-a
            if ($newPhotoPath && $oldPhotoPath && file_exists($oldPhotoPath)) {
                unlink($oldPhotoPath);
            }
            ...
            if($newPhotoPath) {
                 $_SESSION['foto_perfil'] = $newPhotoPath;
            }
            
            $success_message = "Perfil atualizado com sucesso!";

        } catch (PDOException $e) {
            $errors[] = "Erro ao atualizar o perfil: " . $e->getMessage();
        }
    }
}


// Obtém os dados mais recentes do utilizador da sessão para o layout
...
<head>
    ...
    <title>Editar Perfil - MetaXP</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .profile-pic-container { text-align: center; margin-bottom: 2rem; position: relative; }
        .profile-pic { width: 120px; height: 120px; border-radius: 50%; object-fit: cover; margin-bottom: 1rem; border: 4px solid var(--card-bg); box-shadow: 0 0 15px rgba(0,0,0,0.1);}
        #file-upload-label {
        ...
        #file-upload-label:hover { background-color: var(--dark); }
            ...
            <div class="form-card">
                <form action="editar_perfil.php" method="POST" enctype="multipart/form-data">
                    <div class="profile-pic-container">
                        <img src="<?= $fotoPerfil ?>" class="profile-pic" id="profile-pic-preview" alt="Foto de perfil">
                        <br>
                        <label for="foto_perfil" id="file-upload-label">Trocar Foto</label>
                        <input type="file" name="foto_perfil" id="foto_perfil" accept="image/png, image/jpeg, image/gif">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="username" name="username" class="form-control" value="<?= $nomeUsuario ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="email" class="form-label">E-mail</label>
                        <input type="email" id="email" name="email" class="form-control" value="<?= $emailUsuario ?>" required>
                    </div>
